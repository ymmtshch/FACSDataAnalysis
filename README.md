...
- **基本解析ページ（pages/basic_analysis.py）**: 専用の基本解析インターフェース
- **高度解析ページ（pages/advanced_gating.py）**: 複雑なゲーティング機能（開発中）

### データ処理
- **FCSファイル解析**: FCS 2.0、3.0、3.1形式の標準的なフローサイトメトリーファイルの読み込み
- **複数の読み込み方式**: 
  - メインアプリ: 統合処理パイプライン（utils/fcs_processor経由）
  - 基本解析ページ: fcsparserを直接使用した高速処理
...
- **データエクスポート**: 
  - 統計データのCSVダウンロード（固定ファイル名）
  - 表示データ（変換済み・サンプリング済み）のCSVエクスポート
...
│   └── advanced_gating.py   # 高度ゲーティングページ（開発中）
└── utils/
...
#### 📊 ファイル情報・メタデータ
- **メトリクス表示**: イベント数、パラメータ数、取得日の3列表示
- **詳細メタデータ**: 展開可能なセクションで主要メタデータを表示
...
- **自動ファイル名**: 元ファイル名ベースの自動命名

### 4. 高度な機能
- **高度ゲーティング**: `advanced_gating.py`ページで複雑なゲーティング機能を提供（開発中）
...
- **ゲーティング機能**: `utils/gating.py`でゲートタイプを拡張（開発予定）
